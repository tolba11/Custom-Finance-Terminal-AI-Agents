...
def render_price_chart(df: pd.DataFrame, view: str = "Performance",
    ...
    if view == "Performance":
        ...
        fig.add_hline(y=0, line_dash="dot", line_color="#a1a1aa", line_width=1)
        ...
        fig.add_hline(y=start, line_dash="dot", line_color="#a1a1aa", line_width=1)
        ...
        if baseline_price:
            fig.add_hline(y=baseline_price, line_dash="dot",
                          line_color="#a1a1aa", line_width=1,
    ...
    if show_volume and "Volume" in df and view != "Candlestick":
        fig.add_trace(go.Bar(x=df.index, y=df["Volume"], yaxis="y2",
                             marker_color="#e4e4e7", opacity=0.5, name="Volume"))
...
def render_sparkline(closes: pd.Series, baseline: float = None,
    ...
    fig.add_hline(y=base, line_color="#d4d4d8", line_width=0.5, line_dash="dot")
    fig.update_layout(
        template="plotly_white", height=height, showlegend=False,
...
def render_gauge(score: float, title: str, subtitle: str = "",
    ...
    fig = go.Figure(go.Indicator(
        ...
        gauge=dict(
            ...
            bar=dict(color="#18181b", thickness=0.25),
            steps=steps,
            threshold=dict(line=dict(color="#18181b", width=3),
