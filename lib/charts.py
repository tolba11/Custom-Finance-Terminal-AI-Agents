...
def render_price_chart(df: pd.DataFrame, view: str = "Performance",
    ...
    if view == "Performance":
        ...
        fig.add_hline(y=start, line_dash="dot", line_color="#64748b", line_width=1)
    ...
    if show_volume and "Volume" in df and view != "Candlestick":
        fig.add_trace(go.Bar(x=df.index, y=df["Volume"], yaxis="y2",
                             marker_color="#334155", opacity=0.5, name="Volume"))
...
def render_sparkline(closes: pd.Series, baseline: float = None,
    ...
    fig.add_hline(y=base, line_color="#475569", line_width=0.5, line_dash="dot")
    fig.update_layout(
        template="plotly_dark", height=height, showlegend=False,
...
def render_gauge(score: float, title: str, subtitle: str = "",
    ...
    if bands == "risk":
        steps = [
            dict(range=[0, 25], color="#14532d"),
            dict(range=[25, 50], color="#365314"),
            dict(range=[50, 75], color="#713f12"),
            dict(range=[75, 100], color="#7f1d1d"),
        ]
    else:
        steps = [
            dict(range=[0, 33], color="#7f1d1d"),
            dict(range=[33, 66], color="#713f12"),
            dict(range=[66, 100], color="#14532d"),
        ]
    fig = go.Figure(go.Indicator(
        ...
        gauge=dict(
            ...
            bar=dict(color="#e2e8f0", thickness=0.25),
            steps=steps,
